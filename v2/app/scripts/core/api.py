...
class Api(QObject):
    ...
    def __init__(self, page, root_dir):
        ...
        self._progress_dispatcher = ProgressDispatcher(
            lambda *args: self._safe_eval_js("window.updateDownloadProgress", *args)
        )
    ...
    def _download_progress_hook(self, task_id, d):
        ...
        try:
            ...
            if status_key == 'downloading':
                ...
                self._progress_dispatcher.publish(task_id, task_id, percent, status, '', safe_file_arg, task_format)
                ...
                try:
                    ...
                    self._progress_dispatcher.publish(task_id, task_id, percent, "後處理中", '', safe_file_arg, task_format)
                except Exception as e:
    ...
    def _scheduler_status_update(self, task_id, status_text):
        ...
        try:
            with self._lock:
                ...
                task_format = self.task_formats.get(str(task_id), '')
            self._progress_dispatcher.publish(task_id, int(task_id), float(p), str(status_text), '', '', task_format)
        except Exception:
            pass
    ...
    def _notify_download_complete_safely(self, task_id, url, error=None, file_path=None):
        """安全地通知下載完成"""
        try:
            # 先送出被節流的最後一筆進度，避免完成事件後才回寫舊百分比。
            self._progress_dispatcher.flush(task_id)
    ...
    def check_ytdlp_version(self):
        ...
        try:
            ...
            # 檢查快取檔案
            cache_file = safe_path_join(self.root_dir, 'state', 'ytdlp_version_cache.json')
            latest = None
                    ...
                    if ret == 0:
                        ...
                        try:
                            ...
                            if was_updated:
                                ...
                                self._eval_js(f"window.__ofUpdateDone && window.__ofUpdateDone(true, 'yt-dlp 已更新至最新版本！\\n\\n目前版本: {new_version}');")
                            else:
                                self._eval_js("window.__ofUpdateProgress && window.__ofUpdateProgress(100, '更新完成');")
    ...
    def show_update_dialog(self, version_info):
        """顯示更新對話框（完全對齊舊版樣式與互動）"""
        try:
            # 對話框屬於前端責任；後端只傳遞版本資料。這可避免在 UI 執行緒
            # 組裝並注入大量 JavaScript，並讓 overlay 得以重用。
            payload = json.dumps({
                'current_version': version_info.get('current_version', ''),
                'latest_version': version_info.get('latest_version', ''),
            }, ensure_ascii=False)
            self._eval_js(f"window.OldfishUpdateDialog && window.OldfishUpdateDialog.show({payload})")
            return "update"
